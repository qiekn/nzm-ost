...
function MapPlayerPage({ mapData }: { mapData: MapData }) {
  ...
  const bgImage = mapData.background ?? "/images/background_s0.jpg";
  const hasVideo = !!mapData.backgroundVideo;
    ...
    <div className="bg-[#0a0a0a] text-gray-50 h-screen w-screen overflow-hidden flex flex-col relative">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none bg-black">
        <img
          src={asset(bgImage)}
          alt=""
          className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-700 ${hasVideo && videoReady ? "opacity-0" : "opacity-80"}`}
        />
        {hasVideo && (
          <video
            ref={videoRef}
          ...
          >
            <source src={asset(mapData.backgroundVideo!)} type="video/mp4" />
          </video>
        )}
      ...
      <div className={`transition-opacity duration-500 ${hideUI ? "opacity-0 pointer-events-none" : ""}`}>
        <PlayerBar state={state} actions={actions} mapName={mapData.name} mapNameEn={mapData.nameEn} />
      </div>
    </div>
  );
}

export default MapPlayerPage;
